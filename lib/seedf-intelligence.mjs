...
function aggregateLawPerformance(snapshot, laws) {
  ...
  for (const row of questionRows) {
    ...
    if (correct !== null && errors !== null) {
      ...
      if (a !== null && row.date) current.events.push({ date: row.date, accuracy: a });
    }
...
function buildCoverage(laws, bankRows, editalAxes, lawPerformance = []) {
  ...
  const evidencedLawCodes = new Set();
  for (const law of activeLaws) {
    const state = explicitLawProgress(law);
    if (state.started) lawCoverage.studied += 1;
    if (state.practiced) lawCoverage.practiced += 1;
    if (state.evidenced) evidencedLawCodes.add(String(law.code || law.title || ""));
    if (state.consolidated) lawCoverage.consolidated += 1;
  }
  for (const row of lawPerformance) {
    if (row.accuracy !== null) evidencedLawCodes.add(String(row.code || row.title || ""));
  }
  lawCoverage.evidenced = evidencedLawCodes.size;

  const activeAxes = (editalAxes || []).filter(activeAxis);
  const radarAxes = editalAxes.filter((axis) =>
    /radar/.test(norm(axis.documentaryStrength || axis.documentary_strength || axis.layer))
  );
  const outsideAxes = editalAxes.filter((axis) =>
    /fora do escopo/.test(norm(axis.documentaryStrength || axis.documentary_strength || axis.layer))
  );
  const suspendedAxes = editalAxes.filter((axis) => {
    const explicit = norm(axis.documentaryStrength || axis.documentary_strength);
    if (/suspenso/.test(explicit)) return true;
    if (explicit || radarAxes.includes(axis) || outsideAxes.includes(axis)) return false;
    return /monitorar/i.test(String(axis.action || "")) && /monitor/i.test(String(axis.cargos || ""));
  });
  const edital = {
    total: editalAxes.length,
    activeTotal: activeAxes.length,
    radar: radarAxes.length,
    suspended: suspendedAxes.length,
    outside: outsideAxes.length,
    confirmed: editalAxes.filter((axis) => /confirmado pelo tr/.test(norm(axis.documentaryStrength || axis.documentary_strength))).length,
    historical: editalAxes.filter((axis) => {
      const explicit = norm(axis.documentaryStrength || axis.documentary_strength);
      return /base historica forte/.test(explicit) || (!explicit && activeAxis(axis) && /base historica/.test(norm(axis.layer)));
    }).length,
    covered: null,
    domain: null,
    withEvidence: 0,
  ...
  for (const row of lawPerformance) {
    if (row.accuracy !== null && row.discipline) practicedSubjects.add(norm(row.discipline));
  }
...
function buildQuality(snapshot, laws, bankRows, editalAxes) {
  ...
  for (const row of bankRows || []) {
    ...
    const inactive = isStrategicallyInactive(row.strategic_status || row.action || row.priority);
    const started = explicitLawProgress(row).started;
    if (!inactive && started && done === 0 && a === 0) {
...
function buildRisks(activeErrors, reviews, lawPerformance, coverage, interrupted) {
  ...
  const activeEditalTotal = coverage.edital.activeTotal ?? coverage.edital.total;
  if (activeEditalTotal && coverage.edital.covered !== null && coverage.edital.covered / activeEditalTotal < 0.5) {
    risks.push({
      id: "coverage-edital",
      kind: "cobertura",
      title: "Cobertura do edital ainda baixa",
      evidence: [coverage.edital.covered + "/" + activeEditalTotal + " itens ativos marcados como cobertos"],
...
function buildNextAction(snapshot, laws, reviews, activeErrors, interrupted) {
  if (interrupted.length) {
    ...
    return {
      ...
      title: item.code ? item.code + " · retomar sessão incompleta" : item.title,
      reason: `A sessão ${item.code || item.title}${item.date ? " iniciada em " + item.date : " está iniciada"} ainda não foi marcada como concluída. Termine e registre essa etapa antes de avançar a trilha.`,
      href: item.href,
      confidence: "evidência direta",
...
function buildAgenda(snapshot, laws, reviews, interrupted) {
  ...
  const lawAlreadyOpen = currentLaw && interrupted.some((session) => session.code === currentLaw.code);
  if (currentLaw && !lawAlreadyOpen) {
  ...
  const activeDay = snapshot?.execution?.c01?.days?.find((day) => day.day === snapshot?.execution?.c01?.active_day);
  if (activeDay) {
    const waitingOn = interrupted.find((session) => session.code)?.code || null;
    items.push({
      state: waitingOn ? "preparado" : "próximo",
      kind: "sessão",
      title: activeDay.title || activeDay.day,
      detail: waitingOn
        ? `Aguarda concluir a sessão aberta ${waitingOn}; mantenha esta etapa preparada.`
        : activeDay.status || "próxima sessão",
      href: activeDay.href || null,
    });
  }
  const order = { atrasado: 0, hoje: 1, "pendente-sem-data": 2, próximo: 3, preparado: 4 };
...
export function buildSeedfIntelligence(input = {}) {
  ...
  const coverage = buildCoverage(laws, bankRows, editalAxes, lawPerformance);
  ...
  const activeLawQuestions = lawPerformance.length
    ? lawPerformance.reduce((sum, row) => sum + (knownNumber(row.questions) ?? 0), 0)
    : knownNumber(snapshot?.execution?.leis_primeiro?.totals?.done);
