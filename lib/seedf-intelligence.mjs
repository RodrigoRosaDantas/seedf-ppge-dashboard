...
function activeAxis(axis) {
  const explicit = axis?.documentaryStrength || axis?.documentary_strength;
  if (explicit) return !isStrategicallyInactive(explicit);
  const fallback = `${axis?.layer || ""} ${axis?.action || ""} ${axis?.cargos || ""}`;
  if (isStrategicallyInactive(fallback)) return false;
