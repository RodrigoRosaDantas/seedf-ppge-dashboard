...
function buildInterruptedSessions(snapshot, laws) {
  ...
  return sessions
    .filter((session) => session.completed === false && sessionHasExecution(session))
    .map((session) => {
...
function aggregateLawPerformance(snapshot, laws) {
  ...
  return [...groups.values()].map((group) => {
    ...
    const completeness = group.sessions ? group.completeRows / group.sessions : 0;
...
function buildActiveErrors(snapshot, referenceDate) {
  ...
  return raw
    .filter((error) => !isClosedError(error.status))
    .map((error) => {
...
function buildCoverage(laws, bankRows, editalAxes) {
  const lawRows = (bankRows || []).filter((row) => row.record_kind !== "radar");
  const lawCoverage = {
    total: lawRows.length || (laws || []).length,
    available: (laws || []).length,
    studied: 0,
    practiced: 0,
    evidenced: 0,
    consolidated: 0,
  ...
  const edital = { total: editalAxes.length, covered: null, domain: null, withEvidence: 0 };
  const coveredValues = editalAxes.map((axis) => axis.covered).filter((value) => typeof value === "boolean");
  if (coveredValues.length === editalAxes.length && editalAxes.length) edital.covered = coveredValues.filter(Boolean).length;
  const domainValues = editalAxes.map((axis) => axis.domainState).filter(Boolean);
  if (domainValues.length === editalAxes.length && editalAxes.length) {
  ...
  const practicedSubjects = new Set(
    (laws || []).filter((law) => explicitLawProgress(law).evidenced).map((law) => norm(getLawDiscipline(law))),
  );
  edital.withEvidence = editalAxes.filter((axis) => practicedSubjects.has(norm(axis.subject))).length;
  return {
    law: lawCoverage,
    edital,
    note: "Material disponível, estudo, prática, evidência e consolidação são estados distintos.",
  };
}
...
function nextCanonicalLaw(laws) {
  return (laws || []).find((law) => !explicitLawProgress(law).consolidated && !/radar/i.test(String(law.action || law.priority || ""))) || null;
}
...
function buildNextAction(snapshot, laws, reviews, activeErrors, interrupted) {
  ...
  const law = nextCanonicalLaw(laws);
  if (law) {
    return {
      kind: "law",
      score: 70,
...
function buildAgenda(snapshot, laws, reviews, interrupted) {
  ...
  const currentLaw = nextCanonicalLaw(laws);
  if (currentLaw) {
...
export function buildSeedfIntelligence(input = {}) {
  ...
  return {
    ...
    continuity: {
      ...
      law: nextCanonicalLaw(laws)?.code || null,
    },
    methodology: {
      ...
      coverage: "material disponível, estudado, praticado, com evidência e consolidado são estados diferentes",
