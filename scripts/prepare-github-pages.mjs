...
import { studyDisplayStatus } from "../lib/law-study-state.mjs";
...
function buildLawCard(law) {
  ...
  const actions = [
    ...
    law.notion_url ? `<a href="${escapeHtml(law.notion_url)}" target="_blank" rel="noreferrer">Registro vivo · Notion ↗</a>` : "",
  ].join("");
...
<header class="study-topbar"><a href="../">← Leis Primeiro</a><div class="study-topbar-tools"><div class="reading-settings-host" data-reading-settings></div><span class="study-progress">${escapeHtml(law.code)} · ${index + 1} de ${laws.length}</span></div></header>
<section class="study-hero"><p class="study-kicker">⚖️ ${escapeHtml(law.group)} · ${escapeHtml(compactPriority(law.priority))}</p><h1>${escapeHtml(law.code)} — ${escapeHtml(law.title)}</h1><div class="study-meta"><span class="study-chip">🎯 ${law.question_target == null ? "—" : Number(law.question_target)} questões-meta</span><span class="study-chip">Ordem ${escapeHtml(law.operational_order ?? "—")}</span><span class="study-chip">${escapeHtml(studyDisplayStatus(law))}</span></div><div class="study-cargos">${cargos}</div><div class="study-states">${studyStates(law)}</div></section>
...
<div class="study-actions"><a class="primary" href="#study-content">📖 Ler no site</a>${law.official_url ? `<a href="${escapeHtml(law.official_url)}" target="_blank" rel="noreferrer">Fonte oficial ↗</a>` : ""}${law.notion_url ? `<a href="${escapeHtml(law.notion_url)}" target="_blank" rel="noreferrer">Registro vivo · Notion ↗</a>` : ""}${law.bank_record_url ? `<a href="${escapeHtml(law.bank_record_url)}" target="_blank" rel="noreferrer">Registro operacional ↗</a>` : ""}</div>
<article class="study-content">${content}</article>
<nav class="study-nav" aria-label="Navegação entre leis">${prevLink}${nextLink}</nav>
<footer class="study-footer">SEEDF PPGE · leitura principal no site · fonte operacional: Notion · conteúdo sincronizado em ${escapeHtml(formatDate(snapshot.source?.synced_at))}</footer>
</main></body></html>`;
}
...
<section class="laws-hero"><div class="laws-hero-copy"><p class="laws-kicker">⚖️ TRILHA OPERACIONAL · SEEDF PPGE</p><h1>Leis Primeiro</h1><p class="laws-lead">As páginas L01–L34 hospedam a leitura dentro do próprio site. Se o site estiver indisponível, consulte o registro canônico no Notion; para validade jurídica, o texto oficial vigente prevalece.</p><div class="laws-hero-actions"><a class="laws-primary" href="#leis">Abrir primeira lei no site ↓</a><a class="laws-secondary" href="${escapeHtml(snapshot.source?.page_url || "#")}" target="_blank" rel="noreferrer">Registro vivo · Notion ↗</a></div></div><div class="laws-hero-card"><div class="laws-seal">✓</div><div><span>Páginas internas</span><strong>${laws.length} leis</strong><small>L01–L34 · navegação anterior/próxima</small></div></div></section>
