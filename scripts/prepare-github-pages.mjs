...
<section class="study-hero"><p class="study-kicker">⚖️ ${escapeHtml(law.group)} · ${escapeHtml(compactPriority(law.priority))}</p><h1>${escapeHtml(law.code)} — ${escapeHtml(law.title)}</h1><div class="study-meta"><span class="study-chip">🎯 ${law.question_target == null ? "—" : Number(law.question_target)} questões-meta</span><span class="study-chip">Ordem ${escapeHtml(law.operational_order ?? "—")}</span><span class="study-chip">${escapeHtml(law.status || law.action || "")}</span></div><div class="study-cargos">${cargos}</div><div class="study-states">${studyStates(law)}</div></section>
