...
function enhanceLawHtml(html, law) {
  ...
  if (!html.includes("study-reading-layout")) {
    html = html.replace("<article class=\"study-content\">", `<details class="study-mobile-toc"><summary>Sumário desta lei</summary><nav class="study-toc" data-toc-mobile></nav></details><div class="study-reading-layout"><aside class="study-rail"><div class="study-rail-title">Nesta página</div><nav class="study-toc" data-toc-desktop></nav><div class="study-rail-sync">Fonte canônica: Notion<br>Camada de leitura: site<br>Dados: ${esc(bankSnapshot.source?.synced_at ? new Date(bankSnapshot.source.synced_at).toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo"}) : "sincronização automática")}</div></aside><article class="study-content" id="study-content">`);
