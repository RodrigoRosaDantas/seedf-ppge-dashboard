...
function shown(value) { const x = n(value); return x === null ? "—" : String(x); }
...
function cssLink(prefix) { return `<link rel="stylesheet" href="${prefix}leis-enhanced.css?v=20260925c">`; }
function readerCssLink(prefix) { return `<link rel="stylesheet" href="${prefix}reading-preferences.css?v=20260925c">`; }
...
function operationalMarkup(law, row) {
  ...
  const openSessionLabel = openSessions.length === 1 ? "sessão em aberto" : "sessões em aberto";
  const openSessionCompletion = openSessions.length === 1 ? "Ainda não concluída." : "Ainda não concluídas.";
  ...
  return `<section class="study-enhanced-ops" aria-label="Painel operacional sincronizado do Notion">
    ...
    <article class="study-ops-card"><span class="eyebrow">Resumo × lei seca</span><div class="ops-grid"><div class="ops-mini"><b>${summariesDone}</b><small>resumos</small></div><div class="ops-mini"><b>${readingsDone}</b><small>leituras</small></div><div class="ops-mini"><b>${sessionsDone}</b><small>sessões contabilizadas</small></div></div><p class="ops-next">Atual: resumo ${summaryNumber || "—"} · leitura ${readingNumber || "—"}. Estudar esta página não incrementa leitura da norma.</p>${openSessions.length ? `<p class="ops-next">${openSessions.length} ${openSessionLabel}: ${openSessions.map((session) => session.url ? `<a href="${esc(session.url)}" target="_blank" rel="noreferrer">${esc(session.title || `Registro ${law.code}`)} ↗</a>` : esc(session.title || `Registro ${law.code}`)).join(" · ")}. ${openSessionCompletion}</p>` : ""}</article>
...
function enhanceLawHtml(html, law) {
  ...
  if (!html.includes("study-reading-layout")) {
    html = html.replace("<article class=\"study-content\">", `<details class="study-mobile-toc"><summary>Sumário desta lei</summary><nav class="study-toc" data-toc-mobile></nav></details><div class="study-reading-layout"><aside class="study-rail"><div class="study-rail-title">Nesta página</div><nav class="study-toc" data-toc-desktop></nav><div class="study-rail-sync">Fonte canônica: Notion<br>Camada de leitura: site<br>Versão dos dados: ${esc(bankSnapshot.source?.synced_at ? new Date(bankSnapshot.source.synced_at).toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo"}) : "indisponível")}</div></aside><article class="study-content" id="study-content">`);
...
function bankRow(row) {
  ...
  const search = [row.operational_order,row.title,row.priority,row.status,row.action,row.next_step,row.cut,row.alert,row.block,row.observations,...(row.cargos||[]),...codes].filter(Boolean).join(" ").toLocaleLowerCase("pt-BR");
  const details = `${row.cargos?.length?`<div><b>Cargos</b><span>${esc(row.cargos.join(" · "))}</span></div>`:""}<div><b>Progresso real</b><span>${n(row.summaries_done)} resumo(s) · ${n(row.readings_done)} leitura(s) · ${n(row.sessions_done)} sessão(ões)</span></div>${row.cut?`<div><b>Recorte prioritário</b><span>${esc(row.cut)}</span></div>`:""}${row.alert?`<div class="is-alert"><b>Vigência / alerta</b><span>${esc(row.alert)}</span></div>`:""}${row.block?`<div><b>Bloco sugerido</b><span>${esc(row.block)}</span></div>`:""}${row.observations?`<div><b>Observações</b><span>${esc(row.observations)}</span></div>`:""}<div><b>Última leitura / auditoria</b><span>${date(row.last_read)} · ${date(row.last_audit)}</span></div>`;
...
function bankMobileRow(row) {
  ...
  const search = [row.operational_order,row.title,row.priority,row.status,row.action,row.next_step,row.cut,row.alert,row.block,row.observations,...(row.cargos||[]),...codes].filter(Boolean).join(" ").toLocaleLowerCase("pt-BR");
  const details = `<div><b>Progresso real</b><span>${n(row.summaries_done)} resumo(s) · ${n(row.readings_done)} leitura(s) · ${n(row.sessions_done)} sessão(ões)</span></div>${row.cut?`<div><b>Recorte prioritário</b><span>${esc(row.cut)}</span></div>`:""}${row.alert?`<div class="is-alert"><b>Vigência / alerta</b><span>${esc(row.alert)}</span></div>`:""}${row.block?`<div><b>Bloco sugerido</b><span>${esc(row.block)}</span></div>`:""}${row.observations?`<div><b>Observações</b><span>${esc(row.observations)}</span></div>`:""}`;
  const links = `${codes.map((code)=>`<a href="./${esc(code.toLowerCase())}/">${esc(code)} ↗</a>`).join("")}${row.official_url?`<a href="${esc(row.official_url)}" target="_blank" rel="noreferrer">Oficial ↗</a>`:""}${row.url?`<a href="${esc(row.url)}" target="_blank" rel="noreferrer">Notion ↗</a>`:""}`;
  return `<article class="bank-mobile-card" data-bank-mobile-row data-priority="${esc(row.priority||"")}" data-status="${esc(row.status||"")}" data-search="${esc(search)}"><div class="bank-mobile-card-top"><div><span class="bank-order"><b>${esc(row.operational_order)}</b></span><div class="bank-codes">${codeLinks}</div></div><span class="bank-badge priority-${priorityClass(row.priority)}">${esc(row.priority||"—")}</span></div><h3>${esc(row.title||"Sem título")}</h3><div class="bank-mobile-meta"><span>${esc(row.status||"—")}</span><span>·</span><span>${esc(row.action||"—")}</span></div><div class="bank-mobile-metrics"><span><b>${n(row.questions_done)} / ${n(row.question_target)}</b><small>questões</small></span><span><b>${accuracy(row.accuracy)}</b><small>precisão</small></span><span><b>${row.flashcards_done?"✓":"—"}</b><small>flashcards feitos</small></span></div><p class="bank-mobile-next"><strong>Próximo passo:</strong> ${esc(row.next_step||"—")}</p>${details?`<details><summary>Ver recorte e alertas</summary><div class="bank-detail-grid">${details}</div></details>`:""}<div class="bank-links">${links}</div></article>`;
}
