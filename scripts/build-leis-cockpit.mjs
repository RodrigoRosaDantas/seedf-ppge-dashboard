...
function buildExternalRadarItem(radar) {
  return `<article class="laws-radar-item"><div><span class="law-code">R${escapeHtml(radar.operational_order)}</span><strong>${escapeHtml(radar.title)}</strong><small>${escapeHtml(radar.priority || "Radar")} · fora da sequência L01–L34</small></div><div class="laws-radar-actions">${radar.official_url ? `<a href="${escapeHtml(radar.official_url)}" target="_blank" rel="noreferrer">Fonte oficial ↗</a>` : ""}${radar.url ? `<a href="${escapeHtml(radar.url)}" target="_blank" rel="noreferrer">Plano B · Notion ↗</a>` : ""}</div></article>`;
}
...
<details class="laws-panel laws-disclosure laws-audit" id="auditoria"><summary><span><b>🔎 AUDITORIA E INTEGRIDADE</b><strong>O que está preservado</strong></span><span>abrir conferência</span></summary><div class="laws-disclosure-body"><ul>${auditNotes}</ul><p>O site hospeda a leitura local; o Notion é o plano B quando o site estiver indisponível e continua sendo o registro operacional. A fonte jurídica continua sendo o texto oficial vigente.</p></div></details>
<div class="static-note">Leitura principal: snapshots locais publicados no GitHub Pages. Se o site estiver indisponível, use o Notion como fallback. Nenhum controle visual substitui o registro operacional.</div><footer class="laws-footer"><span>SEEDF PPGE · Leis Primeiro</span><span>Leitura principal: site · Fallback: Notion · Fonte jurídica: texto oficial vigente</span></footer></main>
