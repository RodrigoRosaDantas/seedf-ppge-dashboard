...
function percent(done, total) {
  ...
  return d !== null && t !== null && t > 0 ? Math.max(0, Math.min(100, Math.round((d / t) * 100))) : null;
}
...
function stateFor(law, row) {
  ...
  else if (!questionsComplete) nextStep = questionTarget === null || questionsDone === null ? "4 · Questões — amostra ainda não calculável" : `4 · Fazer questões (${questionsDone}/${questionTarget})`;
  ...
  return { complete, questionTarget, questionsDone, flashcardsDone, summariesDone, readingsDone, orientation, d0, d7, d20, nextStep, hasSummary, hasReading, questionsComplete };
}
...
function buildExternalRadarItem(radar) {
  return `<article class="laws-radar-item"><div><span class="law-code">R${escapeHtml(radar.operational_order)}</span><strong>${escapeHtml(radar.title)}</strong><small>${escapeHtml(radar.priority || "Radar")} · fora da sequência L01–L34</small></div><div class="laws-radar-actions">${radar.official_url ? `<a href="${escapeHtml(radar.official_url)}" target="_blank" rel="noreferrer">Fonte oficial ↗</a>` : ""}${radar.url ? `<a href="${escapeHtml(radar.url)}" target="_blank" rel="noreferrer">Registro vivo · Notion ↗</a>` : ""}</div></article>`;
}
...
export async function buildLeisCockpit(sourceHtml) {
  ...
  const mapGroups = [...activeGroups, "Monitor"];
  const groupLabel = (group) => group === "Monitor" ? "Radar — Monitor (acervo)" : group;
  const executable = operationalUnits(laws);
  ...
  const errorMarkup = latestErrors.length
    ? `<div class="laws-map-list">${latestErrors.map((item) => `<article class="laws-map-row"><div class="laws-map-main"><span class="law-code">${escapeHtml(item.question_id || "Erro")}</span><strong>${escapeHtml(item.subject || item.title || "Erro registrado")}</strong></div><div class="laws-map-meta"><span>${escapeHtml(item.review || "Sem revisão")}</span><span>${escapeHtml(item.status || "Sem status")}</span><span>reincidência ${displayNumber(item.recurrence)}</span></div><p class="laws-map-next">${escapeHtml(item.title || item.pattern || item.reason || "Registro vinculado ao Caderno de Erros")}</p>${item.url ? `<a class="laws-map-open" href="${escapeHtml(item.url)}" target="_blank" rel="noreferrer">Abrir erro ↗</a>` : ""}</article>`).join("")}</div>`
    : `<p class="laws-empty">Nenhum erro vinculado a esta execução.</p>`;
  const executionMarkup = latestExecution ? `<section class="laws-panel" id="historico-leis"><div class="laws-heading"><div><p class="laws-kicker">HISTÓRICO REAL · LEIS PRIMEIRO</p><h2>Última execução registrada</h2><p>Resumo, leitura de lei seca, questões e erros permanecem separados pelo Dia ID.</p></div><span class="laws-heading-note">${escapeHtml(latestExecution.day_id)}</span></div><div class="laws-status-strip"><article class="laws-stat-progress"><div class="laws-stat-top"><span class="laws-stat-icon">R</span><span>RESUMO × LEI</span></div><strong>${displayNumber(latestExecution.summary_number)} / ${displayNumber(latestExecution.reading_number)}</strong><small>resumo nº / leitura nº · leitura real não é inferida</small></article><article class="laws-stat-questions"><div class="laws-stat-top"><span class="laws-stat-icon">Q</span><span>QUESTÕES</span></div><strong>${displayNumber(latestExecution.done)}/${displayNumber(latestExecution.planned)}</strong><small>${displayNumber(latestExecution.correct)} acertos · ${formatExecutionPercent(latestExecution.precision)}</small></article><article class="laws-stat-map"><div class="laws-stat-top"><span class="laws-stat-icon">E</span><span>CADERNO DE ERROS</span></div><strong>${latestErrors.length}</strong><small>erros vinculados por Dia ID</small></article><article class="laws-stat-source"><div class="laws-stat-top"><span class="laws-stat-icon">D0</span><span>D0 DA NORMA</span></div><strong>${latestLaw?.d0 ? "Concluído" : "Pendente"}</strong><small>Flashcards: ${latestLaw?.flashcards_done ? "feitos" : "pendentes"} · sem meta numérica</small></article></div>${errorMarkup}</section>` : "";
...
<header class="laws-topbar"><a class="laws-back" href="../">← Dashboard SEEDF</a><div class="laws-topbar-tools"><div class="reading-settings-host" data-reading-settings></div><div class="laws-sync"><span class="laws-live-dot"></span> Versão dos dados · ${escapeHtml(formatDate(snapshot.source?.synced_at))}</div></div></header>
<section class="laws-hero laws-cockpit-hero"><div class="laws-hero-copy"><p class="laws-kicker">⚖️ TRILHA OPERACIONAL · SEEDF PPGE</p><h1>Leis Primeiro<span class="laws-hero-dot">.</span></h1><p class="laws-lead">A fila de leitura, questões e revisão do SEEDF. O Notion é a fonte operacional canônica; o site hospeda a leitura e organiza a execução.</p><div class="laws-hero-thesis"><span>LER</span><i>→</i><span>RESPONDER</span><i>→</i><span>REVISAR</span></div><div class="laws-hero-meta"><span class="laws-live-dot"></span><span>Fonte canônica: Notion</span><span class="laws-meta-separator">·</span><span>Camada de leitura: site</span></div><div class="laws-hero-actions"><a class="laws-primary" href="${escapeHtml(currentHref)}">▶️ Continuar ${escapeHtml(current?.code || "a trilha")}</a><a class="laws-secondary" href="#trilha">Ver trilha ↓</a><a class="laws-secondary" href="./flashcards/">🧠 Abrir flashcards</a><a class="laws-secondary" href="${escapeHtml(snapshot.source?.page_url || "#")}" target="_blank" rel="noreferrer">Registro vivo · Notion ↗</a></div></div><aside class="laws-next-card"><div class="laws-next-top"><div><p class="laws-kicker">PRÓXIMA AÇÃO</p><span>Bloco atual · ${escapeHtml(current?.code || "—")}</span></div><span class="laws-next-badge">${String(currentStage).padStart(2, "0")} / 06</span></div><div class="laws-next-law"><span class="laws-next-code">${escapeHtml(current?.code || "—")}</span><h2>${escapeHtml(current?.title || "Aguardando dados")}</h2></div><div class="laws-next-context"><span>${current ? `${current.code} / ${laws.length}` : "—"}</span><span>${displayNumber(currentState.questionTarget)} questões-meta</span><span>${escapeHtml(current?.group || "Aguardando")}</span></div><div class="laws-next-focus"><span>${String(currentStage).padStart(2, "0")}</span><div><small>FAÇA AGORA</small><strong>${escapeHtml(currentState.nextStep || "Aguardando sincronização")}</strong></div></div><p>Resumo e leitura de lei seca são eventos distintos. Feche D0 somente após cumprir os requisitos reais da Lxx. D7/D20 seguem em paralelo.</p><div class="laws-checkpoints">${currentChecks}</div><a class="laws-next-cta" href="${escapeHtml(currentHref)}">Abrir norma <span aria-hidden="true">↗</span></a><a class="laws-next-notion" href="${escapeHtml(currentNotion)}" target="_blank" rel="noreferrer">Registro vivo · abrir no Notion ↗</a></aside></section>
