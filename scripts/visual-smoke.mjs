    ...
    for(const [routeName,route] of routes){
      for(const [width,height] of sizes){
        ...
        if(routeName==="leis"){
          ...
          if(!(radarReadiness?.roleLabels||[]).includes("Núcleo comum")||!(radarReadiness?.roleLabels||[]).includes("Gestor — Administração")||!(radarReadiness?.roleLabels||[]).includes("Analista de Apoio Administrativo")) failures.push("leis "+width+"x"+height+" nome de cargo ausente no mapa de leis");
          if(!/L11.*L25.*L28–L34.*R901/.test(radarReadiness?.radarNote||"")) failures.push("leis "+width+"x"+height+" acervo fora da fila diária incompleto");
          if(radarReadiness?.chipColor!=="rgb(22, 123, 109)") failures.push("leis "+width+"x"+height+" contador do Radar com contraste incorreto: "+radarReadiness?.chipColor);
